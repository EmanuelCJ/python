...
class InventarioApp:
    ...
    def crear_interfaz(self):
        ...
        self.crear_campo(input_frame, "Buscar por descripcion:", self.descripcion_buscar_var , 1)
    ...
    def validar_datos_busqueda(self):
        ...
        # Validar que no haya campos vacíos
        if not self.id_buscar_var.get() and not self.descripcion_buscar_var.get():
        ...
        if not isinstance(self.descripcion_buscar_var.get(), str):
            messagebox.showwarning("Advertencia", "El campo Descripción debe ser texto.")
            return False
    ...
    # Retirar producto osea cantidad de stock
    ...
    def tipo_busqueda(self):
        ...
        btn_descripcion = tk.Button(ventana, text="Por descripción", width=20, command=lambda: [self.eleccion.set("descripcion"), ventana.destroy()])
    ...
    def encontrar_coincidencia(self):
        ...
        try:
            descripcion = self.descripcion_buscar_var.get()
            
            # Cargar datos actuales osea dataframe del excel
            tabla_excel = pd.read_excel(self.archivo_excel)
            
            # Buscar producto
            coincidencias = tabla_excel[tabla_excel["Descripción"].str.contains(descripcion, case=False)] 
